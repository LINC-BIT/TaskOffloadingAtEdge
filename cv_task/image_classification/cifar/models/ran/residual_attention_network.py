...
def ran_92_32(num_classes=100):
    return ResidualAttentionModel_92_32input_update(num_classes=num_classes)
def RAN(num_classes=10, in_channels=3):
    return ResidualAttentionModel_92_32input_update(num_classes, in_channels=in_channels)
...
if __name__=='__main__':
    ...
    model.load_state_dict(torch.load("/data/zql/legodnn-on-var-models/xgf_model/cifar10/ran/model_92_sgd.pkl"))
    import sys
    sys.path.insert(0, '/data/zql/zedl')
    from zedl.dl.common.model import get_model_flops_and_params, get_model_size
    from zedl.dl.common.pruning import l1_prune_model
    
    print(get_model_size(model, True))
    print(get_model_flops_and_params(model, (1, 3, 32, 32)))

    pruned_model = l1_prune_model(model, None, 0.8, (1, 3, 32, 32), 'cpu', dep_aware=True)
