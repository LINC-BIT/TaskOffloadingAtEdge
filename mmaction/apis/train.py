...
def train_model(model,
    ...
    if method is not None:
        if method=='usnet':
            ...
            runner.run(data_loaders, cfg.workflow, cfg.total_epochs, method=method, width_mult_list=width_mult_list, sample_net_num=sample_net_num, **runner_kwargs)
            # cal
            ...
            runner.run(data_loaders, cfg.workflow, cfg.total_epochs, method=method, fn3_all_layers=fn3_all_layers, fn3_disable_layers=fn3_disable_layers, min_sparsity=min_sparsity, **runner_kwargs)
            
        elif method=='cgnet':
            gtar = cfg.get('gtar', None)
            input_size = cfg.get('input_size', None)
            assert gtar is not None and input_size is not None
            runner.run(data_loaders, cfg.workflow, cfg.total_epochs, method=method, gtar=gtar, input_size=input_size, **runner_kwargs)
        elif method=='nestdnn':
            grad_positions = cfg.get('grad_positions', None)
            # freeze = cfg.get('freeze', None)
            assert grad_positions is not None
            runner.run(data_loaders, cfg.workflow, cfg.total_epochs, method=method, grad_positions=grad_positions, **runner_kwargs)
        elif method=='nestdnnv3':
            zero_shape_info = cfg.get('zero_shape_info', None)
            # freeze = cfg.get('freeze', None)
            assert zero_shape_info is not None
            runner.run(data_loaders, cfg.workflow, cfg.total_epochs, method=method, zero_shape_info=zero_shape_info, **runner_kwargs)
        else:
